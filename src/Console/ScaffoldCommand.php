...
class ScaffoldCommand extends Command
{
    ...
    protected array $stub_path = [
        ...
        'view-index-header-line' => __DIR__ . '/../../stubs/views/view-index-header-line.stub',
        'view-index-body-line' => __DIR__ . '/../../stubs/views/view-index-body-line.stub',
    ...
    protected function makeViewIndex(): self
    {
        $replacements = array_merge($this->replacements, [
            ...
            '{{ colspan }}' => count($this->properties) + 1,

            '{{ header }}' => collect($this->properties)->map(function ($value) {
                return trim($this->getContents($this->getStubPath('view-index-header-line'), [
                    '{{ model }}' => $this->toSnake($this->model),
                    '{{ field }}' => $value['name'],
                ]));
            })->implode("\n".str_repeat(self::INDENT, 7)),

            '{{ body }}' => collect($this->properties)->map(function ($value) {
                return trim($this->getContents($this->getStubPath('view-index-body-line'), [
                    '{{ model }}' => $this->toSnake($this->model),
                    '{{ field }}' => $value['name'],
                ]));
            })->implode("\n".str_repeat(self::INDENT, 7)),
